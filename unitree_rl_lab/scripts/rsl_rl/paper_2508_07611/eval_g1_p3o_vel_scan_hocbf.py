"""Record evaluation videos for the isolated realG1 velocity-scan P3O-HOCBF checkpoint."""
...
parser = argparse.ArgumentParser(description="Evaluate a realG1 velocity-scan P3O-HOCBF checkpoint.")
...
parser.add_argument("--topdown_overview", action="store_true", default=False, help="Use a static top-down overview camera for the full terrain.")
...
parser.add_argument("--history_length", type=int, default=3, help="Observation history length.")
parser.add_argument("--lidar_num_rays", type=int, default=120, help="Number of rays in the compressed 2D distance scan.")
...
parser.add_argument("--terrain_obstacles", type=int, default=20, help="Number of terrain protrusion obstacles when --terrain_clutter is enabled.")
parser.add_argument(
    "--terrain_layout",
    type=str,
    default="surrounded_front_open",
    choices=[
        "lanes",
        "surround_wide",
        "surround_hybrid",
        ...
        "end2end_frontclose",
        "arena_walled",
        "surrounded_front_open",
        "ring_passages",
        "dense_irregular",
    ],
    help="Terrain protrusion layout used for preview videos.",
)
...
parser.add_argument("--cmd_vx", type=float, default=0.22)
...
def set_camera(env):
    robot = env.unwrapped.scene["robot"]
    if args_cli.topdown_overview:
        env_origins = env.unwrapped.scene.env_origins.cpu()
        center = env_origins.mean(dim=0)
        env.unwrapped.sim.set_camera_view(
            eye=[center[0].item() + 2.5, center[1].item(), center[2].item() + max(args_cli.terrain_size_y, 18.0)],
            target=[center[0].item() + 2.5, center[1].item(), center[2].item()],
        )
        return
...
def set_realg1_observations(env_cfg):
    ...
    params = {"num_rays": args_cli.lidar_num_rays, "max_distance": args_cli.lidar_max_distance, "fov_deg": args_cli.lidar_fov_deg}
    scale = tuple([1.0 / args_cli.lidar_max_distance] * args_cli.lidar_num_rays)
...
def build_actor_specs() -> list[ObsTermSpec]:
    ...
    return [
        ...
        ObsTermSpec("obstacle_scan", args_cli.lidar_num_rays, h),
