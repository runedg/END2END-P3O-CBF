...
parser.add_argument("--resume", type=str,
    default="/home/ubuntu/P3O-CBF/logs/P3O-END2END-001/2026-04-29_12-52-49/model_final.pt")
...
def main():
    ...
    # 4) Balance gait quality, speed tracking and safety
    env_cfg.rewards.alive.weight = 0.30
    env_cfg.rewards.base_height.weight = -8.0          # relaxed: allow natural height variation
    env_cfg.rewards.flat_orientation_l2.weight = -5.0
    env_cfg.rewards.action_rate.weight = -0.05         # relaxed: smoother gait transitions
    env_cfg.rewards.joint_vel.weight = -0.001
    env_cfg.rewards.energy.weight = -1.0e-5            # very relaxed energy penalty
    env_cfg.rewards.joint_deviation_waists.weight = -1.5
    env_cfg.rewards.joint_deviation_legs.weight = -1.2
    env_cfg.rewards.gait.weight = 0.0                  # disabled: focus purely on speed tracking
    env_cfg.rewards.feet_slide.weight = -0.15
    env_cfg.rewards.feet_clearance.weight = 1.2        # slightly reduced: avoid excessive lift
    env_cfg.rewards.undesired_contacts.weight = -0.8
    env_cfg.rewards.track_lin_vel_xy.weight = 3.5      # increased: force speed tracking
    env_cfg.rewards.track_ang_vel_z.weight = 1.5       # increased: better heading tracking

    # 5) Command range: higher minimum speed to prevent slowing-down shortcut
    cmd_cfg = env_cfg.commands.base_velocity
    cmd_cfg.rel_standing_envs = 0.02
    cmd_cfg.ranges.lin_vel_x = (0.20, args_cli.max_vx)
    ...
    print(f"[INFO] Balanced rewards: gait={env_cfg.rewards.gait.weight}, feet_clearance={env_cfg.rewards.feet_clearance.weight}, action_rate={env_cfg.rewards.action_rate.weight}, base_height={env_cfg.rewards.base_height.weight}, track_lin_vel={env_cfg.rewards.track_lin_vel_xy.weight}")
