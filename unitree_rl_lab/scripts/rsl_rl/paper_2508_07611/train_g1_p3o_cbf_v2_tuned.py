...
"""Isolated v2-tuned realG1 Mid360 P3O-CBF trainer.

This script is a modified copy of `train_g1_p3o_cbf_realG1_paper_curriculum.py`.
It keeps the original training script untouched and focuses on three problems
identified from P3O-END2END-001/2026-04-30_06-06-44:

1. Command tracking speed was too slow in evaluation.
2. Obstacle avoidance was too conservative and started avoiding too early.
3. Overall posture / gait looked stiff and unnatural.

Primary changes vs the original script:
- Narrower safety margin and softer CBF tightening rate.
- Higher speed command curriculum and stronger linear-velocity tracking weight.
- Softer joint-deviation / orientation penalties to release posture.
- Higher gait and feet-clearance reward to improve naturalness.
- Lower comfort-related penalty weights so the robot does not over-avoid.
"""
...
# ── argparse ────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="V2-tuned realG1 Mid360 P3O-CBF trainer.")
parser.add_argument("--num_envs", type=int, default=4096, help="Number of environments.")
parser.add_argument("--max_iterations", type=int, default=12000, help="Training iterations.")
parser.add_argument("--save_interval", type=int, default=1000, help="Save checkpoint every N iterations.")
parser.add_argument("--experiment_name", type=str, default="P3O_V2_Tuned", help="Top-level log folder name.")
...
# CBF / obstacle tuning
parser.add_argument("--safety_margin", type=float, default=0.55, help="Safe distance to obstacles.")
parser.add_argument("--cbf_gamma", type=float, default=0.35, help="Discrete CBF coefficient.")
parser.add_argument("--collision_distance", type=float, default=0.18, help="Distance counted as collision.")
...
parser.add_argument("--contact_cost_weight", type=float, default=2.5, help="Extra P3O cost for non-foot body contacts.")
...
# - Stage 0 keeps a small warmup standing ratio so the policy starts gently.
# - Speed command range is pushed higher than v1 to cover near-eval speeds.
# - Track weights are stronger; comfort penalties are softer.
CURRICULUM_STAGES = (
    {
        "name": "stage0_single_front_warmup",
        "progress": 0.0,
        "num_obstacles": 1,
...
def _configure_base_env(env_cfg):
    ...
    # v2 reward base: softer joint/orientation penalties, stronger gait terms.
...
def main():
    ...
    if args_cli.resume:
        ...
        print(f"[INFO] Resumed from {args_cli.resume} at iteration {start_iteration}")
