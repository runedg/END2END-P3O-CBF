...
def build_deploy_cfg(template_cfg: dict, args) -> dict:
    ...
    point_dim = args.num_fps_points * 3
    observations = {
        ...
        "lidar_points": {
            ...
            "clip": [-args.lidar_max_distance, args.lidar_max_distance],
            "scale": [1.0 / args.lidar_max_distance] * point_dim,
    ...
    for term_name in observations:
        if term_name != "lidar_points":
            observations[term_name]["history_length"] = args.history_length

    cfg["commands"]["base_velocity"]["ranges"]["lin_vel_x"] = [0.10, 0.45]
    cfg["commands"]["base_velocity"]["ranges"]["lin_vel_y"] = [-0.08, 0.08]
    cfg["commands"]["base_velocity"]["ranges"]["ang_vel_z"] = [-0.30, 0.30]
...
def main():
    parser = argparse.ArgumentParser(description="Export pointcloud-based paper-curriculum checkpoint for isolated sim2real deployment.")
    ...
    policy = ActorCriticSafePerception(
        actor_term_specs=build_actor_term_specs(args.history_length, args.num_fps_points),
        critic_term_specs=build_critic_term_specs(args.history_length, args.num_fps_points),
