...
"""Isolated velocity-tracking G1 P3O-CBF training with 2D scan perception.

This experiment intentionally avoids goal-point observations. It trains stable
velocity tracking and obstacle avoidance first, using a fixed 2D distance scan
that can later be supplied by a DUNE/NEUPAN-like real perception frontend.
"""
...
parser = argparse.ArgumentParser(description="Train an isolated G1 velocity-tracking P3O-HOCBF policy with 2D scan.")
...
parser.add_argument("--experiment_name", type=str, default="P3O-VEL-SCAN-CBF-001", help="Top-level log folder name.")
...
parser.add_argument("--cbf_gamma", type=float, default=0.5, help="Legacy env CBF coefficient; HOCBF cost uses alpha1/alpha2.")
...
parser.add_argument("--cbf_cost_weight", type=float, default=1.0, help="Weight for high-order CBF violation.")
parser.add_argument("--collision_cost_weight", type=float, default=2.0, help="Extra weight for near-collision states.")
parser.add_argument("--hocbf_alpha1", type=float, default=1.6, help="First high-order CBF class-K coefficient.")
parser.add_argument("--hocbf_alpha2", type=float, default=1.2, help="Second high-order CBF class-K coefficient.")
parser.add_argument("--hocbf_accel_clip", type=float, default=8.0, help="Clip finite-difference h_ddot for HOCBF stability.")
parser.add_argument("--hocbf_warmup_steps", type=int, default=8, help="Rollout steps before enabling h_ddot in HOCBF.")
parser.add_argument("--contact_cost_weight", type=float, default=4.0, help="Extra P3O cost for non-foot body contacts.")
parser.add_argument("--contact_force_threshold", type=float, default=1.0, help="Contact force threshold for non-foot contact cost.")
parser.add_argument("--curriculum_max_obstacles", type=int, default=4, help="Maximum number of obstacles to reveal.")
parser.add_argument("--fixed_active_obstacles", type=int, default=None, help="Use a fixed number of active obstacles.")
parser.add_argument("--lidar_num_rays", type=int, default=180, help="Number of rays in the compressed 2D distance scan.")
parser.add_argument("--lidar_fov_deg", type=float, default=360.0, help="Horizontal field-of-view of the 2D distance scan.")
...
class HOCBFCostState:
    """Finite-difference state for the isolated velocity-scan HOCBF experiment."""

    def __init__(self):
        self.prev_h_dot: torch.Tensor | None = None
        self.calls = 0

    def compute(self, env, cfg):
        ...
        if self.prev_h_dot is None or self.prev_h_dot.shape != h_dot.shape:
            raw_h_ddot = torch.zeros_like(h_dot)
        else:
            raw_h_ddot = (h_dot - self.prev_h_dot.to(device=h_dot.device)) / max(dt, 1.0e-6)

        if self.calls < cfg.hocbf_warmup_steps:
            h_ddot = torch.zeros_like(raw_h_ddot)
        else:
            h_ddot = torch.clamp(raw_h_ddot, -cfg.hocbf_accel_clip, cfg.hocbf_accel_clip)

        self.prev_h_dot = h_dot.detach().clone()
        self.calls += 1

        psi1 = h_dot + cfg.hocbf_alpha1 * h
        hocbf_margin = h_ddot + cfg.hocbf_alpha2 * psi1
        hocbf_violation = torch.relu(-hocbf_margin)
        ...
        total_cost = (
            ...
            + cfg.cbf_cost_weight * hocbf_violation
        ...
        info = {
            ...
            "paper/psi1_min": psi1.min().item(),
            "paper/hocbf_margin_mean": hocbf_margin.mean().item(),
            "paper/hocbf_margin_min": hocbf_margin.min().item(),
            "paper/hocbf_violation_rate": (hocbf_violation > 0).float().mean().item(),
            "paper/cbf_violation_rate": (hocbf_violation > 0).float().mean().item(),
...
_hocbf_cost_state = HOCBFCostState()


def compute_paper_cost(env, cfg):
    return _hocbf_cost_state.compute(env, cfg)
...
def apply_obstacle_curriculum(obstacle_manager, cfg, stage_progress: float) -> int:
    ...
    if cfg.fixed_active_obstacles is not None:
        active = min(max(1, cfg.fixed_active_obstacles), max_active)
...
def _set_dense_lidar_observations(env_cfg, num_rays: int, max_distance: float, fov_deg: float, history_length: int):
    ...
    scale = tuple([1.0 / max_distance] * num_rays)
