...
"""Independent realG1 Mid360 P3O-HOCBF curriculum experiment.

This script intentionally lives next to, but does not modify, the baseline
P3O-CBF curriculum script. P3O remains unchanged; only the CBF-derived cost is
replaced by a high-order CBF cost for isolated ablation experiments.
"""
...
parser = argparse.ArgumentParser(description="Train an isolated realG1 Mid360 P3O-HOCBF policy with curriculum.")
...
parser.add_argument("--experiment_name", type=str, default="P3O-END2END-HOCBF-001", help="Top-level log folder name.")
...
parser.add_argument("--hocbf_alpha1", type=float, default=2.0, help="First high-order CBF gain for psi1 = h_dot + alpha1 * h.")
parser.add_argument("--hocbf_alpha2", type=float, default=2.0, help="Second high-order CBF gain for h_ddot + alpha2 * psi1.")
parser.add_argument("--hocbf_accel_clip", type=float, default=8.0, help="Absolute clamp for finite-difference h_ddot to reduce geometry jitter.")
parser.add_argument("--hocbf_warmup_steps", type=int, default=4, help="Initial cost calls with zeroed h_ddot after startup.")
...
class HOCBFCostState:
    """Finite-difference state for the isolated high-order CBF experiment."""
